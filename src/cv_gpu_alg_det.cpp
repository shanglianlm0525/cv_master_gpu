...
/*
CvGpuAlgDet::CvGpuAlgDet(InPara& iparam, std::string model_path, std::vector<unsigned char>& model_data)
{
    m_param = iparam;
...
void CvGpuAlgDet::init(InPara& iparam, std::string model_path, std::vector<unsigned char>& model_data)
{
    m_param = iparam;
...
void CvGpuAlgDet::preprocess(std::vector<cv::Mat> imgsBatch)
{
...
void CvGpuAlgDet::postprocess(std::vector<cv::Mat> imgsBatch)
{
    ...
    decodeDevice(m_param, m_output_src_transpose_device, 4 + m_param.num_class, m_total_objects, m_output_area,
        m_output_objects_device, m_output_objects_width, m_param.topK, 0);
    // nms
    // nmsDeviceV1(m_param, m_output_objects_device, m_output_objects_width, m_param.topK, m_param.nChannel, m_param.topK * m_output_objects_width + 1);
    nmsDeviceV2(m_param, m_output_objects_device, m_output_objects_width, m_param.topK, m_param.nChannel, m_param.topK * m_output_objects_width + 1, m_output_idx_device, m_output_conf_device);
    checkRuntime(cudaMemcpy(m_output_objects_host, m_output_objects_device, m_param.batch_size * sizeof(float) * (1 + m_output_objects_width * m_param.topK), cudaMemcpyDeviceToHost));
}


int CvGpuAlgDet::infer(std::vector<cv::Mat> imgsBatch, ModelPara mPara, DetectInfo* dInfoList, int* dNum)
{
    assert(imgsBatch.size() <= m_param.batch_size);
