...
void resize_rgb_without_padding_device_kernel(float* src, int src_width, int src_height, int src_area, int src_volume,
	float* dst, int dst_width, int dst_height, int dst_area, int dst_volume,
	int batch_size, AffineMat matrix)
{
	int dx = blockDim.x * blockIdx.x + threadIdx.x;
	int dy = blockDim.y * blockIdx.y + threadIdx.y;
	if (dx < dst_area && dy < batch_size)
...
__global__
void resize_gray_padding_device_kernel(float* src, int src_width, int src_height, int src_area,
	...
	int batch_size, float padding_value, AffineMat matrix)
{
	...
	if (dx < dst_area && dy < batch_size)
	{
		...
		else
		{
			...
			int y_high = y_low + 1;
			int x_high = x_low + 1;
			float const_values[] = { padding_value };
			float ly = src_y - y_low;
			float lx = src_x - x_low;
			float hy = 1 - ly;
			float hx = 1 - lx;
		...
		float* pdst = dst + dy * src_area + dst_y * dst_width * 1 + dst_x * 1;
		pdst[0] = c0;
...
__global__
void resize_gray_padding_device_kernel(unsigned char* src, int src_width, int src_height, int src_area,
	float* dst, int dst_width, int dst_height, int dst_area,
	int batch_size, float padding_value, AffineMat matrix)
{
	int dx = blockDim.x * blockIdx.x + threadIdx.x;
...
void norm_device_kernel(float* src, float* dst,
	int batch_size, int img_height, int img_width, int img_channel, int img_area, int img_volume,
	float scale,
	...
	float std0, float std1, float std2)
{
	...
	if (dx < img_volume && dy < batch_size)
	{
		int ch = dx % img_channel;
		assert(ch < img_channel);
	...
	int batch_size, int img_height, int img_width, int img_channel, int img_area, int img_volume)
{
	...
	if (dx < img_volume && dy < batch_size)
	{
		...
		int dx_ = row * (img_width * img_channel) + col * img_channel + ch;
	...
	float paddingValue, AffineMat matrix)
{
	...
	int src_volume = srcChannel * srcHeight * srcWidth;
	int src_area = srcHeight * srcWidth;

	int dst_volume = srcChannel * dstHeight * dstWidth;
	...
	if (srcChannel == 3) {
		...
		resize_gray_padding_device_kernel << < grid_size, block_size, 0, nullptr >> > (src, srcWidth, srcHeight, src_area,
			dst, dstWidth, dstHeight, dst_area,
			batchSize, paddingValue, matrix);
	}
}

void resizeDevice(const int& batchSize, unsigned char* src, int srcWidth, int srcHeight, int srcChannel,
	float* dst, int dstWidth, int dstHeight,
	float paddingValue, AffineMat matrix)
{
	dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
...
void bgr2rgbDevice(const int& batchSize, float* src, int srcWidth, int srcHeight, int srcChannel,
	float* dst, int dstWidth, int dstHeight)
{
	dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
	dim3 grid_size((dstWidth * dstHeight * srcChannel + BLOCK_SIZE - 1) / BLOCK_SIZE,
		(batchSize + BLOCK_SIZE - 1) / BLOCK_SIZE);

	int img_volume = srcChannel * srcHeight * srcWidth;
...
void normDevice(const int& batchSize, float* src, int srcWidth, int srcHeight, int srcChannel,
	float* dst, int dstWidth, int dstHeight,
	InPara param)
{
	dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
	...
	norm_device_kernel << < grid_size, block_size, 0, nullptr >> > (src, dst, batchSize, img_height, img_width, srcChannel, img_area, img_volume,
		param.scale, param.means[0], param.means[1], param.means[2], param.stds[0], param.stds[1], param.stds[2]);

}

void hwc2chwDevice(const int& batchSize, float* src, int srcWidth, int srcHeight, int srcChannel,
	float* dst, int dstWidth, int dstHeight)
{
	dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
	dim3 grid_size((dstWidth * dstHeight * srcChannel + BLOCK_SIZE - 1) / BLOCK_SIZE,
	...
	int img_height = srcHeight;
	int img_width = srcWidth;
	hwc2chw_device_kernel << < grid_size, block_size, 0, nullptr >> > (src, dst, batchSize, img_height, img_width, srcChannel, img_area, img_volume);
}


__global__ void softmax_kernel(float* src, int batch_size, int num_class) {
	...
	if (idx < batch_size) {
		...
		for (int i = 0; i < num_class; i++) {
			if (src[idx * num_class + i] > max_val) {
				max_val = src[idx * num_class + i];
			}
		}

		// Compute the exponential of each element and accumulate the sum
		for (int i = 0; i < num_class; i++) {
			float exp_val = expf(src[idx * num_class + i] - max_val);
			src[idx * num_class + i] = exp_val;
			sum += exp_val;
		}

		// Normalize each element by the sum
		for (int i = 0; i < num_class; i++) {
			src[idx * num_class + i] /= sum;
		}
	}
}
...
{
	dim3 block_size(BLOCK_SIZE, BLOCK_SIZE);
	dim3 grid_size((batch_size + BLOCK_SIZE - 1) / BLOCK_SIZE);

	softmax_kernel << < grid_size, block_size >> > (src, batch_size, num_class);
}
